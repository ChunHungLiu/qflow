...
if ( -d ${projectpath}/tech/${techname} ) then
   ...
   if ( -d ${projectpath}/${techname} ) then
      set techdir=${topdir}/${techname}
   else
      if ( -d ${projectpath}/tech && -r ${projectpath}/tech/${techname}.sh ) then
	 set techdir=${projectpath}/tech
      else
	 if ( -d /usr/local/share/qflow/tech/${techname} ) then
	    set techdir=/usr/local/share/qflow/tech/${techname}
	 else
            echo "Don't know about technology '${techname}'"
	    echo "Check /usr/local/share/qflow/tech/ for known technologies"
	 endif
      endif
   endif
endif

# Set variables from install locations here, so we don't have to do it in every
# script separately.  Track path to scripts and binaries used by qflow
